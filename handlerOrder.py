...
def checkPnl(threadId, args):
    ...
    if positionAmount == 0:
        ...
        if pnl > 1:
            mesaj = "------KÂR {}$------\nCoin Sembol: {}\nİşlem Tipi: {}\nİşlem Id: {}\nMiktar: {}\nHarcama: {}\nPNL: {}\nDaha fazla risk almamak için işlemden çıkmanızı öneriyoruz.".format(
                pnl, symbol, orderType, orderId, quantity, spent, pnl)
            sendTelegram(str(userConfig['telegramChatId']), mesaj)
            sendLog(mesaj)
        elif pnl < -1:
                    ...
                    for a in coinData['apiKeys']:
                        ...
                        if str(side).lower() == "BUY".lower():
                            ...
                            if str(userConfig['short']) == "false":
                                ...
                                return
                        sendLog(
                            'Finished to check long and short trade settings.')
                        if availableBalance < alimButce:
                            ...
                            return
                        sendLog(
                            'Finished to check availible budget is enough for trade.')
                        sameOrder = [
                            x for x in openOrders if x['coin'] == str(coin).upper()]
                        samePosition = [
                            x for x in positionOrders if x['coin'] == str(coin).upper()]
                        if len(samePosition) > 1 or len(sameOrder) > 0:
                            ...
                            return
                        sendLog(
                            'Finished to check any open or position order with same coin.')
                        apiOpenOrders = [
                            x for x in openOrders if x['apiId'] == int(apiId)]
                        if len(apiOpenOrders) == int(userConfig['maxActiveOrders']):
                            ...
                            return
                        sendLog(
                            'Finished to check order count is higher than setted maximum order count.')
                        try:
                            sendLog("Order Request Sending to Binance for user:" +
                            ...
                            if "2021" in e:
                                if side == "SELL":
                                    sendLog(
                                        "ERROR: Activation price should be smaller than current price.")
                                elif side == "BUY":
                                    sendLog(
                                        "ERROR: Activation price should be larger than current price.")
                                return "immediately"
                            elif "1111" in e:
                                return "pres"
            break
...
def alimYap(threadId, args):
    ...
    sendLog(mesaj)
    order = createOrder({"coin": coin, "side": orderTypex,
                        "coinAlimEmriDeger": coinAlimEmriDeger, "pres": int(args['pres']), "type": "all"})
    if order == "immediately":
        while True:
            ...
            order = createOrder({"coin": coin, "side": orderTypex, "coinAlimEmriDeger": coinAlimEmriDegerx,
                                "pres": pres, "type": str(args['type']), "type": "all"})
            if order == "immediately":
                continue
            elif order == "pres":
                break
    ...
    for dirs in os.walk("users"):
        ...
        for user in usersArray:
            ...
            for position in positionOrders:
                ...
                createThread("pnl", {"orderId": orderId, "symbol": symbol, "side": side, "coin": coin, "orderType": orderType,
                             "coinAlimEmriDeger": coinAlimEmriDeger, "quantity": quantity, "spent": spent, "userId": userId, "apiId": apiId})
            ...
            for order in openOrders:
                ...
                createThread("check", {"orderId": orderId, "symbol": symbol, "side": side, "coin": coin, "orderType": orderType,
                             "coinAlimEmriDeger": coinAlimEmriDeger, "quantity": quantity, "spent": spent, "userId": userId, "apiId": apiId})
